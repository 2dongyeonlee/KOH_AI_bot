...
• D-N 업무명 / 담당자
규칙:
- D-0·D-1이면 날짜 표기 생략 (D-0 = 오늘이라 자명)
- D-2·D-3이면 "(MM/DD)" 간략히 표기
...
📋 방별 주요 내용 (최근 3일)
각 방에서 오간 내용 중 업무 관련 핵심만 방별로 1~2줄 요약.
잡담·인사는 제외. 방 이름을 앞에 표시.
예: [6R전략_w/권] 인디애나 FAB 기공식 TM 참석 여부 논의
없으면 특이사항 없음.`;
  ...
  if (isFileReq) {
    ...
    if (fileHits.length) {
      for (const fh of fileHits) {
        const src = fh.room_title ? ` (${fh.room_title})` : "";
        await sendDocument(env, chatId, fh.file_id,
          `요청하신 자료입니다.${src} ${fh.file_name || ""}`.trim());
      }
  ...
  const rows = (
    await env.DB.prepare(
      `SELECT content, sender_name, summary, action_items, needs_escalation, status_tag, milestone_date, created_at,
        CASE
          WHEN milestone_date >= date('now') AND milestone_date <= date('now', '+3 days') THEN 1
          ELSE 0
        END AS is_due_soon,
        CASE
...
=== [방별 내용] ===
${recentMsgs.map(r => `[${r.room_title || "미상"}] ${r.sender_name || ""}: ${(r.content || "").slice(0, 100)}`).join("\n").slice(0, 3000) || "특이사항 없음"}`,
    "",
    MODEL_SMART
  );
